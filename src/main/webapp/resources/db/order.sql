			...
			AND S.TEAM='관리팀'
			AND O.REGDATE<![CDATA[>=]]>TO_DATE(#{firstdate}, 'YYYYMMDDHH24MISS')
			AND O.REGDATE<![CDATA[<=]]>TO_DATE(#{seconddate}, 'YYYYMMDDHH24MISS')
			AND O.PROCODE=#{product}
			AND O.EMPNO=#{emp}
...
SELECT O.ID AS ID, O.EMPNO AS EMPNO, O.REGDATE AS REGDATE,
	   ...
	   (SELECT EMPNO, NAME, TEAM, AUTH FROM SMEMBER) S,
	   (SELECT PROCODE, PRONAME, PROAMOUNT, ORIGINPRICE, SELLPRICE FROM PRODUCT) P
	   WHERE O.EMPNO=S.EMPNO AND O.PROCODE=P.PROCODE AND TEAM='영업1팀'
