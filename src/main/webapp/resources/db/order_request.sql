...
alter table OREQUEST drop (OURL, OPRICE);
...
DELETE FROM OREQUEST WHERE EMPNO='NULL'
...
SELECT ONUM, MAX(REGDATE) AS REGDATE, COUNT(*) AS CNT, MAX(EMPNO) AS EMPNO, MAX(PROCODE) AS PROCODE FROM 
(SELECT * FROM 
...
O.CHECKS as CHECKS , O.DELETED as DELETED, S.TEAM as TEAM  FROM OREQUEST O, SMEMBER S WHERE O.EMPNO = S.EMPNO) 
WHERE TEAM='관리팀')
